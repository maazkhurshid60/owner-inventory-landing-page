...
const Tooltip: React.FC<ToolTipProps> = ({ text, isComparisonToolTip = false }) => {
    return (
        <div className={`${isComparisonToolTip
            ?
            "absolute left-[-20px] bottom-[120%] z-[999] hidden group-hover:flex flex-col items-start"
            :
            ...
            <div className={`${isComparisonToolTip
                ...
                :
                "rounded-xl border border-[#795DF5] bg-white px-4 py-2 text-sm leading-4 font-onest font-semibold text-[#231F20] shadow-lg w-fit"}`}>
                {text}
            </div>
            {isComparisonToolTip &&
